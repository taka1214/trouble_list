          ...
          <section class="text-gray-600 body-font">
            <div class="container mx-auto">
              ...
              <div class="lg:w-3/4 w-full mx-auto overflow-auto">
                <!-- 検索フォームstart -->
                <form method="get" action="{{ route('admin.expired-users.index') }}" class="flex">
                ...
                <table class="table-auto w-full mb-5 text-left whitespace-no-wrap mt-5">
                  ...
                  <tbody>
                    ...
                    <tr class="sm:hidden">
                      <td class="px-2 md:px-4 py-3 cursor-pointer" onclick="location.href='{{ route('admin.expired-users.show', ['user' => $user->id]) }}'">{{ $user->name }}</td>
                      <td class="px-2 md:px-4 py-3 cursor-pointer" onclick="location.href='{{ route('admin.expired-users.show', ['user' => $user->id]) }}'">{{ $user->deleted_at->format('m/d/H:i') }}</td>
                    </tr>
                    <tr class="hidden sm:table-row">
                      ...
                      <td class="px-2 sm:px-4 py-3 text-center">
                        <form id="delete_{{ $user->id }}" method="post" action="{{ route('admin.expired-users.restore', ['user' => $user->id]) }}">
                          @csrf
                          @method('put')
