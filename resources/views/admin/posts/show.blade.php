                ...
                <div class="-m-2">
                  ...
                  <div class="p-2 w-full mt-4 flex flex-col lg:justify-center sm:flex-row justify-around">
                    <div class="flex justify-around mb-2 sm:mb-0">
                      <form method="get" action="{{ route('admin.posts.edit', ['post' => $post->id ]) }}">
                      ...
                      <form id="delete_{{ $post->id }}" method="post" action="{{ route('admin.posts.destroy', ['post' => $post->id ]) }}" class="lg:ml-5 md:ml-5">
                        @csrf
                        @method('DELETE')
                        ...
                        <li id="body{{ $reply->id }}" class="w-full rounded bg-gray-100 bg-opacity-50 border-b-2 border-gray-100 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out list-none">
                          <div class="show_reply">
                            ...
                            <div class="flex justify-end mt-2">
                              ...
                              <form id="destroy_{{ $reply->id }}" method="post" action="{{ route('admin.replies.destroy', ['reply' => $reply->id ]) }}" class="text-white bg-alert border-0 py-1 px-2 focus:outline-none hover:bg-opacity-90 rounded text-sm mr-2">
                                @csrf
                                @method('DELETE')
                          ...
                          <div id="edit{{ $reply->id }}" class="hidden">
                            <form method="post" action="{{ route('admin.replies.update', ['reply' => $reply->id ]) }}" enctype="multipart/form-data">
                              @csrf
                              @method('PUT')
                  ...
                  <div class="p-2 w-full mt-4 flex justify-around">
                    <button type="button" onclick="location.href='{{ route('admin.posts.index') }}'" class="bg-gray-300 border-0 py-2 px-8 focus:outline-none hover:bg-opacity-90 rounded-xl text-md shadow-md">一覧に戻る</button>
                  </div>
                </div>
              </div>
            </div>
