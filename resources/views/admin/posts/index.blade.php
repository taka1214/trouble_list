<x-app-layout>
          ...
          <section class="text-gray-600 body-font">
            <div class="container mx-auto">
              ...
              <div class="lg:w-3/4 w-full mx-auto overflow-auto">
                <!-- 検索フォームstart -->
                <form method="get" action="{{ route('admin.posts.index') }}" class="flex flex-wrap">
                ...
                <div class="flex justify-end mb-4 mt-3">
                  <button onclick="location.href='{{ route('admin.posts.create') }}'" class="text-white  bg-default border-0 py-1 px-6 mt-3 focus:outline-none  hover:bg-opacity-90 rounded-xl text-md shadow-md">
                    新規登録
                  </button>
                </div>
                <table class="table-auto w-full mb-5 text-left whitespace-no-wrap">
                  ...
                  <tbody>
                    ...
                    @endphp
                    <tr class="{{ $bgColor }}">
                      <td class="px-2 sm:px-4 py-3" onclick="location.href='{{ route('admin.posts.show', ['post' => $post->id]) }}'">
                        <div>{{ Str::limit($post->title, 30, '…' ) }}</div>
                        <div class="sm:hidden text-xs ml-2">{{ Str::limit($post->body, 80, '…' ) }}</div>
                      </td>
                      <td class="hidden sm:table-cell px-2 sm:px-4 py-3" onclick="location.href='{{ route('admin.posts.show', ['post' => $post->id]) }}'">{{ Str::limit($post->body, 80, '…' ) }}</td>
                      ...
                      <td class="px-2 sm:px-4 py-3 text-center md:block hidden">
                        <button onclick="location.href='{{ route('admin.posts.show', ['post' => $post->id]) }}'" class="text-white bg-default border-0 py-1 px-3 focus:outline-none hover:bg-opacity-90 rounded-xl text-md shadow-md">詳細</button>
                      </td>
  ...
  <script>
    window.likeUrl = "{{ route('admin.posts.like', ['id' => '__id']) }}";
    window.unlikeUrl = "{{ route('admin.posts.unlike', ['id' => '__id']) }}";
  </script>
