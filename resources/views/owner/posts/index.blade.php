<x-app-layout>
          ...
          <section class="text-gray-600 body-font">
            <div class="container mx-auto">
              ...
              <div class="lg:w-3/4 w-full mx-auto overflow-auto">
                <!-- 検索フォームstart -->
                <form method="get" action="{{ route('owner.posts.index') }}" class="flex flex-wrap">
                ...
                <div class="flex justify-end mb-4 mt-3">
                  <button onclick="location.href='{{ route('owner.posts.create') }}'" class="text-white  bg-default border-0 py-1 px-6 mt-3 focus:outline-none  hover:bg-opacity-90 rounded-xl text-md shadow-md">
                    新規登録
                  </button>
                </div>
                <table class="table-auto w-full mb-5 text-left whitespace-no-wrap">
                  <thead>
                    <tr>
                      <th class="px-2 md:px-4 py-3 title-font tracking-wider font-medium text-gray-900 text-sm bg-gray-100 rounded-tl rounded-bl">タイトル</th>
                      <th class="hidden md:table-cell px-2 md:px-4 py-3 title-font tracking-wider font-medium text-gray-900 text-sm bg-gray-100 rounded-tl rounded-bl">本文</th>
                  ...
                  <tbody>
                    ...
                    <tr>
                      <td class="px-2 sm:px-4 py-3 cursor-pointer" onclick="location.href='{{ route('owner.posts.show', ['post' => $post->id]) }}'">{{ Str::limit($post->title, 30, '…' ) }}</td>
                      <td class="hidden md:table-cell px-2 sm:px-4 py-3" onclick="location.href='{{ route('owner.posts.show', ['post' => $post->id]) }}'">{{ Str::limit($post->body, 60, '…' ) }}</td>
                      ...
                      <td class="px-2 sm:px-4 py-3 text-center md:block hidden">
                        <button onclick="location.href='{{ route('owner.posts.show', ['post' => $post->id]) }}'" class="text-white bg-default border-0 py-1 px-3 focus:outline-none hover:bg-opacity-90 rounded-xl text-md">詳細</button>
                      </td>
  ...
  <script>
    window.likeUrl = "{{ route('owner.posts.like', ['id' => '__id']) }}";
    window.unlikeUrl = "{{ route('owner.posts.unlike', ['id' => '__id']) }}";
  </script>
