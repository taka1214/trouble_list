                  ...
                  <div class="p-2 w-full mt-4 flex flex-col lg:justify-center sm:flex-row justify-around">
                    ...
                    <div class="flex justify-around mb-2 sm:mb-0">
                      <form method="get" action="{{ route('owner.posts.edit', ['post' => $post->id ]) }}">
                        <button type="submit" class="text-white bg-indigo-500 border-0 py-2 px-8 ml-1 focus:outline-none hover:bg-indigo-600 rounded text-lg">編集</button>
                      </form>
                      <form id="delete_{{ $post->id }}" method="post" action="{{ route('owner.posts.destroy', ['post' => $post->id ]) }}">
                        @csrf
                        @method('DELETE')
                        <button data-id="{{ $post->id }}" onclick="return deletePost(this)" class="text-white bg-red-500 border-0 py-2 px-8 ml-1 focus:outline-none hover:bg-red-600 rounded text-lg">削除</button>
                      </form>
                    </div>
                    @endif
                          ...
                          <div class="show_reply">
                            ...
                            <div class="flex justify-end mt-2">
                              ...
                              <form id="destroy_{{ $reply->id }}" method="post" action="{{ route('owner.replies.destroy', ['reply' => $reply->id ]) }}">
                                @csrf
                                @method('DELETE')
                                <button data-id="{{ $reply->id }}" onclick="return deletePost(this)" class="text-white bg-red-500 border-0 py-1 px-2 focus:outline-none hover:bg-red-600 rounded text-lg">削除</button>
                              </form>
                            </div>
                            @endif
